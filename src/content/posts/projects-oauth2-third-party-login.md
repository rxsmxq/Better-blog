...
> [!NOTE] 提示
> 本文统一整理 GitHub、微信和 QQ 第三方登录的 OAuth 2.0 授权码流程，覆盖授权 URL、state 校验、回调换取令牌、用户信息映射、策略模式和多环境配置。实现重点是防止 CSRF、避免把第三方令牌暴露给浏览器，以及隔离不同平台的差异。

## 方案边界

本文以服务端回调为主，未覆盖 PKCE、账号绑定冲突处理、第三方平台审核流程和生产密钥轮换；这些内容需要根据客户端类型和平台要求补充。
